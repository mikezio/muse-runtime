...
**Browser** - text search, page fetching and the live-browser task system (`spawn_task` for new work, `steer_task` for continuations). Reading and doing are separate tools with separate rules. See [Browser](05-browser.md).

**Subagents** - delegate bounded work to child agents: spawn, list, send follow-up input, close. The tree is managed, not fire-and-forget. See [Agents](08-agents.md).
...
**Credentials (Secure Vault)** - request a login capture or API-key setup, list saved logins (metadata only). The agent gets a link to a secure page; the secret itself never passes through chat or files. See [Safety](07-safety.md).
